...
resource "local_file" "invoke_ansible_playbook" {
  count = var.vm_count
  content = azurerm_linux_virtual_machine.my_linux_vm[count.index].public_ip_address   
  ...
  depends_on = [
    null_resource.install_nginx_web_server_on_azure_vms,
    local_file.private_key_file
  ]
}
